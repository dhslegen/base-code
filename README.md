...
```bash
base-code init    # 在当前目录生成教学型 base-code.yaml 模版（已存在时报错，--force 覆盖）
```

模版中必填三项已填示例值、可选项全部注释并标注默认值——编辑后直接 `base-code gen` 即可。
...
> 完整教学型模版可由 `base-code init` 一键生成（与下方样例同源），无需从零手写。
